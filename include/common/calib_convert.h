...
namespace SY3_NAMESPACE
{
	...
	//	//otheroffset4 Spareパラメータ
	//	for (int i = 0; i < proto_calib.otheroffset4_size(); i++){
	//		//Sleep(1);
	//	}

	//	//otheroffset5 Spareパラメータ
	//	for (int i = 0; i < proto_calib.otheroffset4_size(); i++){
	//		//Sleep(1);
	//	}
	//	//周波数80MHzでwiggling_lut_lower_lenパラメータが20より大きい場合、wiggling幅は50mmになります。それ以外の場合は100mmとします。
	//	//シングル周波数は別にします。シングル周波数40MHzはwiggling半周期までしかキャリブレーションしていないためです。paraments_calib.freq_lower = 80e6 &&
